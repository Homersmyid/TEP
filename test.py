...
def theta_rule1(mod,i,j):
	return (mod.b[i,j] * (mod.theta[i] - mod.theta[j])
		>= mod.tran[i,j] - (1 - mod.x_star[i,j]) * mod.Mtran		-33)
...
def ref_rule2(mod):
	return mod.theta[mod.ref] == 0
mod.RefConstraint = Constraint(rule=ref_rule2)
# Reference Theta
#	Theta_refernce = 0
def ref_rule(mod):
	return mod.theta[mod.ref] == 0
...
def lag_trans(mod,i,j):
	return ((mod.flow_dual[i] - mod.flow_dual[j] - mod.theta_dual[i,j]
			+ mod.capmax_dual[i,j] - mod.capmin_dual[i,j]) * mod.x_star[i,j] == 0)
